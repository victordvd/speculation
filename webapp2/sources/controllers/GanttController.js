...
var GanttController = /** @class */ (function () {
    function GanttController() {
    }
    GanttController.processLotData = function (output) {
        var newOutput = [];
        //add dummy task
        ...
        for (var i = 0; i < output.length; i++) {
            var task = output[i];
            var seq = task.seq;
            var dummy = task.dummy;
            if (dummy) {
                var dummyTask = Object.assign({}, task);
            ...
            if (task.parent == 0) {
                ...
                var curJob = task.prod_id + '-' + step;
                var curJobText = (task.text) ? task.text.substring(2, 4) + '<br>' + step : '';
                var curEqp = task.parent_text;
